...
const TASK_NAME_USER: &str = "YIZ Gateway";
const TASK_NAME_ADMIN: &str = "YIZ Gateway (Admin)";
...
async fn cleanup_legacy_shortcuts() -> Result<()> {
    ...
    let old_shortcut = startup_dir.join("YIZ-Gateway.lnk");
    let new_shortcut = startup_dir.join("YIZ Gateway.lnk");
